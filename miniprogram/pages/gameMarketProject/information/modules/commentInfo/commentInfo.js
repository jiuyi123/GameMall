...
Page({
  ...
  async LoadInfo() {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      let time = []
      for (let a = 0; a < 4; a++) {
        year.push(list[i].Comment.Time[a])
      }
      if (list[i].Comment.Time[5] != 0) {
        month.push(list[i].Comment.Time[5])
      }
      month.push(list[i].Comment.Time[6])
      month.push('月')
      if (list[i].Comment.Time[8] != 0) {
        date.push(list[i].Comment.Time[8])
      }
      date.push(list[i].Comment.Time[9])
      date.push('日')
      for (let j = 11; j < 16; j++) {
        time.push(list[i].Comment.Time[j])
      }
